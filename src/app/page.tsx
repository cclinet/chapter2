...
export default function Page() {
  ...
  function textAreaChange(e: ChangeEvent<HTMLTextAreaElement>) {
    const str = e.target.value;
    const lines = str.split(/\r\n|\r|\n/).length;
    textareaRef.current!.rows = 1 + lines;
    textareaRef.current!.style.height = `${38 + lines * 20}px`;
  }
  return (
    <div className="flex h-full min-h-screen flex-1 flex-col">
      <div className="flex h-full w-full flex-1 flex-col items-center lg:justify-between">
        ...
        <div className="flex h-full w-full flex-col overflow-y-auto bg-zinc-100 md:flex-row md:overflow-y-hidden lg:pl-[260px]">
          ...
          <div className="@container h-full min-h-[250px] w-full min-w-[280px] overflow-hidden  bg-zinc-50">
            <div className="flex h-full flex-col flex-nowrap overflow-y-auto">
              ...
              <div className="min-w-0 flex-1">
                <div className="scrolling-touch scrolling-gpu relative h-full w-full overflow-auto overscroll-y-auto pb-12">
                  {messages.map(({ content, role, createdAt, id }) => (
                    <div>{content}</div>
                  ))}
                </div>
              </div>
              <div className="sticky bottom-0 z-10 min-h-0 min-w-0 flex-shrink-0">
                ...
                >
                  <textarea
                    ...
                    className={
                      "h-[38px] w-full flex-1 resize-none scroll-m-2 overflow-y-hidden rounded-md border border-zinc-200/80 bg-zinc-200/50 pb-12 text-base transition-colors focus:border-zinc-400 focus:outline-none focus:ring-0 sm:text-sm"
                    }
                    onChange={(e) => {
                      handleInputChange(e), textAreaChange(e);
                    }}
                  />
                  <div
                    className={
